...
function getLead($lead_id, $acc_id, $lm_initializer_id)
{
    ...
    if (isset($lm_initializer_id)){
        $populateLeadPostData['lm_initializer_id'] = $lm_initializer_id;
...
function getPolicyLeadByTicketID($acc_id, $leadTicket){
    $leadImKey = '3765d732472d44469e70a088caef3040';

    $populateLeadPostData = [
        'key' => $leadImKey,
        'acc_id' => $acc_id,
        'searchby' => "102158",         //ticketNumber
        "campaign"  => "17967",         //Policy campaign
        'searchterm' => $leadTicket
    ];

    $leadInfoStr = httpPost('http://proxy.leadim.xyz/apiproxy/acc3305/searchlead.ashx' , $populateLeadPostData);
    $leadToPopulateJson = json_decode($leadInfoStr, true);
    if ($leadToPopulateJson['lead_id'] > 0){
        return getLead($leadToPopulateJson['lead_id'], $acc_id, null);
    }
    return null;
}
