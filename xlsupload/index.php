...
if (isset($_FILES)) {
    ...
    if (is_uploaded_file($_FILES['file']['tmp_name'])) {
        if ($type == 1) {
            ...
            foreach ($a as $file) {
                if (($file != ".") && ($file != "..")) {
                    if (md5_file("user-upload-test/" . $file) == md5_file($_FILES['file']['tmp_name'])) {
                        $flag = 1;
                    }
                }
            }
...
?>
    <script type="text/javascript">
        ...
        $.ajax({
            ...
            type: "post",
            data: {
                'acc_id':<?php print $crmAccount; ?>,
                'usr_id':<?php print $agentid; ?>,
                'lead_id':<?php print $leadid; ?>,
            ...
            <div class="col-xs-12 col-md-12 col-sm-12">
                <fieldset>
                    <div class="col-xs-6 col-md-6 col-sm-6">
                        <a href="<?php print (isset($_SERVER["HTTPS"]) ? "https://" : "http://") . $_SERVER['SERVER_NAME']; ?>/user-upload/<?php print $leadid; ?>/<?php print $newnameimg; ?>"
                           class="button_center btn btn-default">הצג מסמך</a>
                    </div>
                    <div class="col-xs-6 col-md-6 col-sm-6">

                        <a href="<?php print (isset($_SERVER["HTTPS"]) ? "https://" : "http://") . $_SERVER['SERVER_NAME']; ?>/?recordNumber=<?php print $leadid; ?>&crmAcccountNumber=<?php print $crmAccount; ?>&agentId=<?php print $agentid; ?>&fileType=<?php print $files ?>&name=<?php print $_POST['name']; ?>&phone=<?php print $_POST['phone']; ?>"
                           class="button_center btn btn-default"> הוסף מסמך נוסף</a>

                        <button type="button" id="back_to_form" class="button_center btn btn-default">הוסף מסמך נוסף
                        </button>
                    </div>
                </fieldset>
            </div>
                ...
                <form action="redirect.php" method="post" id="uploadForm" enctype="multipart/form-data">
                    <fieldset>
                        ...
                        <div class="form-group">
                            <input type="hidden" class="input-group form-control"
                                   value="<?php if ($_GET['recordNumber']) {
                                       print $_GET['recordNumber'];
                                   } ?>" name="recordNumber"/>
                            <input type="hidden" class="input-group form-control"
                                   value="<?php if ($_GET['crmAcccountNumber']) {
                                       print $_GET['crmAcccountNumber'];
                                   } ?>" name="crmAcccountNumber"/>
                            <input type="hidden" class="input-group form-control" value="<?php if ($_GET['agentId']) {
                                print $_GET['agentId'];
                            } ?>" name="agentId"/>
                            <input type="hidden" class="input-group form-control"
                                   value="<?php print $customerFullName ?>" name="name"/>
                            <input type="hidden" class="input-group form-control" value="<?php print $customerPhone ?>"
                                   name="phone"/>
                        </div>
                        <div class="form-group">
            ...
            <div class="col-xs-12 col-md-12 col-sm-12">
                <fieldset>
                    ...
                    <div class="col-xs-4 col-md-4 col-sm-4">
                        ...
                        <a href="<?php print (isset($_SERVER["HTTPS"]) ? "https://" : "http://") . $_SERVER['SERVER_NAME']; ?>/list.php"
                           class="button_center btn btn-default"> List</a>
                    ...
                    <fieldset>
                        ...
                        <div class="form-group">
                            <input type="hidden" class="input-group form-control"
                                   value="<?php if ($_GET['recordNumber']) {
                                       print $_GET['recordNumber'];
                                   } ?>" placeholder="מספר הליד" name="recordNumber"/>

                            <input type="hidden" class="input-group form-control"
                                   value="<?php if ($_GET['crmAcccountNumber']) {
                                       print $_GET['crmAcccountNumber'];
                                   } ?>" placeholder="מספר חשבון" name="crmAcccountNumber"/>
                            <input type="hidden" class="input-group form-control" value="<?php if ($_GET['agentId']) {
                                print $_GET['agentId'];
                            } ?>" placeholder="מספר נציג" name="agentId"/>
                            <input type="hidden" class="input-group form-control"
                                   value="<?php print $customerFullName ?>" name="name"/>
                            <input type="hidden" class="input-group form-control" value="<?php print $customerPhone ?>"
                                   name="phone"/>
                            <input type="hidden" class="input-group form-control" value="<?php if ($_GET['fileType']) {
                                print $_GET['fileType'];
                            } ?>" name="fileType"/>
                        </div>
                        <div class="form-group">
